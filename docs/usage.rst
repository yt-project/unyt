...
Below we list all of the equivalencies that are built into ``unyt`` and
illustrate their use with a short example. For full API documnentation and
an autogenerated listing of the built-in equivalencies in ``unyt``, see
the :mod:`unyt.equivalencies` API listing.
