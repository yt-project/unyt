...
7. When you're done making changes, check that your changes pass linting,
   and run the tests
    ...
    $ uv run --group test pytest --doctest-modules --doctest-rst --doctest-plus
...
1. The pull request should include tests for functionality that is not already
   ...
   new untested code. You can generate coverage reports locally by running
   ``pytest`` through ``coverage`` as ``uv run --group covcheck coverage run -m pytest``
